...
\end{minipage}
		...
		En el ejemplo 2, se busca una 2-PMP del grafo $G_{1}$.  Dicho grafo consta de un subgrafo completo bipartito,
		conformado por los conjuntos de nodos A y B, y de un nodo $u$, unido mediante una arista con todos los nodos
		del conjunto A. Cada arista que une nodos de A con B tiene peso INF, mientras que cada arista que une al nodo
		$u$ con nodos del conjunto A, tiene peso 1. La solución óptima a este problema esta presentada en la segunda
		figura, y es de peso $M$, con $M < \infty$, mientras que la solución que brinda nuestra heurística es de peso
		infinito, y una posible representación de la misma esta esquematizada en la primera figura. La heurística falla
		porque vamos agregando los nodos a cada conjunto en un orden que nos conduce a una solución no óptima. En este caso,
		los nodos del conjunto A son los que primero van a ser agregados a conjuntos. Entonces, como nuestro algoritmo
		es goloso y cada vez que agrega un nodo a un conjunto, lo agrega al conjunto que minimiza el hazard nuevo que se
		genera, seguro que van a haber al menos dos nodos $x,y \in A$ tales que $x$ sea agregado al conjunto 1 e $y$ al
		conjunto 2. Esto hace que, más adelante, para cada nodo del conjunto B, como está unido tanto a $x$ como a $y$, 
		no importa si lo agregamos al conjunto 1 o al conjunto 2, en ambos casos el hazard extra que se genera
